...
export default function AddFriendButton({userId}: {userId: string}){
    ...
    const { data: receivedRequests = [] } = useGetFromRequestUser({ userId: currentUser?.uid || "" });

    const isFriend = friends.some((friend) => friend.id === userId)
    const isSent= sentRequests.some((sent) => sent.id === userId)
    const isReceived= receivedRequests.some((received) => received.id === userId)
